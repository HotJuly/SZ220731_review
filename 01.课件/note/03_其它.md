...
- 2XX: 表示成功处理请求
  ...
  - 201->代表请求成功,但是参数不对
- 3XX: 需要重定向, 浏览器直接跳转
  - 302->代表地址重定向,我们请求A网址,但是A服务器把我引导到B网址去
  - 304->代表是协商缓存,我们第二次请求服务器的某个资源,服务器发现,浏览器内存中还有该文件,那么就返回响应,状态码为304,不再次返回该资源,会让浏览器去自己的内存中,找到上次的数据进行复用
- 4XX: 客户端请求错误
  - 401->代表当前身份标识过期,token过期了,需要重新登录
  - 403->代表权限不够,也就是说服务器有这个资源,但是不给你
  - 404->代表服务器没有该资源,这个问题大概率是前端接口地址写错了
- 5XX: 服务器端错误
  - 500->代表当前服务器无法处理该请求,服务器崩溃了
  - 501->代表当前请求方式不对,出现该问题,对比下自己项目的请求方式和接口文档是否相同

#### 区别ajax请求与一般HTTP请求

注意:

​	ajax不是一种特殊的请求类型,他只是一种发送请求的工具,发出去的还是http请求
...
1. 配置通用的基础路径和超时时间
   1. 配置基础路径可以延伸到跨域的解决方案proxy
   2. 超时时间可以延伸到请求的最大并发数量限制
      1. 小程序请求最大并发限制是10个,默认超时时间是60s
      2. Chrome和Firefox请求最大并发限制是6个,超时时间设置一般是5s,10s,20s
         1. 如果请求超时,那么该请求状态码就会变为404,并自动终止
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
7. 对token过期的错误进行处理
   1. 服务器如果发现token过期了,会将本次请求终止,并把响应状态码设置为401
...
1. DNS 解析(就是将一个域名解析成对应的服务器IP地址)
   1. 域名的了解
      1. 好处:方便记忆,方便推广,降低推广成本
      2. 坏处:相比于直接使用IP访问服务器,域名还需要解析,更加浪费时间
         1. 属于性能的劣化
...
- layout 布局(排版)：计算元素的位置和大小信息
  - 重排:当页面上元素的大小或者位置相关信息发生变化,就会发生重排
- render 渲染(绘制)：将颜色/文字/图片等渲染上去
  - 重绘:当页面上元素的颜色,文字,图片等内容发生变化,就会发生重绘
  - 重排一定伴随着重绘,重绘不一定伴随着重排
  - 扩展:提高页面渲染的性能优化方案
    - 开启图层
      - video/audio/canvas等标签都具有自带图层
      - 使用了transform,transition,animation等C3新特性的元素也会自动开启图层
      - 给元素添加样式will-change:true,即可强行开启图层
